...
func TestBuyDevCardDeductsResourcesAndAddsCard(t *testing.T) {
	...
	if err := BuyDevCard(state, "P1"); err != nil {
	...
	if p.VictoryPointCards != 1 {
		t.Errorf("expected VictoryPointCards 1, got %d", p.VictoryPointCards)
	}
}

func TestBuyDevCardInsufficientResources(t *testing.T) {
	state := &pbb.GameState{Players: []*pbb.PlayerState{{Id: "P2", Resources: &pbb.ResourceCount{Ore: 0, Wheat: 1, Sheep: 1}}}}
	if err := BuyDevCard(state, "P2"); err == nil {
		t.Fatalf("expected error for insufficient resources, got nil")
	}
}

func TestPlayDevCardDecrementCount(t *testing.T) {
	state := &pbb.GameState{Players: []*pbb.PlayerState{{Id: "P3", VictoryPointCards: 1, DevCardCount: 1}}}
	err := PlayDevCard(state, "P3", pbb.DevCardType_DEV_CARD_TYPE_VICTORY_POINT)
	if err != nil {
...
func TestPlayDevCardNoCardError(t *testing.T) {
	state := &pbb.GameState{Players: []*pbb.PlayerState{{Id: "P4", VictoryPointCards: 0, DevCardCount: 1}}}
	err := PlayDevCard(state, "P4", pbb.DevCardType_DEV_CARD_TYPE_VICTORY_POINT)
	if err == nil {
